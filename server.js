...
app.post('/api/auth/send-otp', async (req, res) => {
  try {
    const { contactMethod, type } = req.body; // type = 'email' | 'phone'
    if (!contactMethod) return res.status(400).json({ message: 'Contact method required' });

    const otp = Math.floor(100000 + Math.random() * 900000).toString();
    const otpExpiry = new Date(Date.now() + 10 * 60000); // 10 mins

    let user = await User.findOne(type === 'email' ? { email: contactMethod } : { phone: contactMethod });
    if (!user) {
      user = new User(type === 'email' ? { email: contactMethod } : { phone: contactMethod });
    }
    user.otp = otp;
    user.otpExpiry = otpExpiry;
    await user.save();

    console.log(`[OTP GENERATED] for ${contactMethod}: ${otp}`);

    if (type === 'email' && process.env.EMAIL_USER && process.env.EMAIL_PASS) {
      try {
        await transporter.sendMail({
          from: process.env.EMAIL_USER,
          to: contactMethod,
          subject: 'Your Ankri Candles Login Code',
          text: `Your login code is: ${otp}. It will expire in 10 minutes.`
        });
      } catch (mailErr) {
        console.error('Mail sending failed, continuing with fallback:', mailErr);
      }
    }
    res.status(200).json({ message: 'OTP sent successfully', dummyCode: (!process.env.EMAIL_USER || process.env.EMAIL_USER === 'placeholder') ? otp : null });
  } catch (err) {
...
app.post('/api/auth/verify-otp', async (req, res) => {
  try {
    const { contactMethod, otp, type } = req.body;
    const user = await User.findOne(type === 'email' ? { email: contactMethod } : { phone: contactMethod });

    if (!user || user.otp !== String(otp) || user.otpExpiry < new Date()) {
    ...
    const token = jwt.sign({ id: user._id, email: user.email, phone: user.phone }, JWT_SECRET, { expiresIn: '30d' });
...
app.get('/api/user/orders', async (req, res) => {
  try {
    ...
    if (decoded.email) queryConditions.push({ 'customerInfo.email': decoded.email });
    if (decoded.phone) queryConditions.push({ 'customerInfo.phone': decoded.phone });

    if (queryConditions.length === 0) return res.status(200).json([]);

    const bookings = await Booking.find({ $or: queryConditions }).sort({ timestamp: -1 });
