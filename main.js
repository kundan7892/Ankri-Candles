...
function initAppFlow() {
  ...
  homeTabButtons.forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      if (homepageDashboardTitle) {
        ...
        if (activeHomeTab === 'customers') homepageDashboardTitle.textContent = "Registered Customers";
      }
  ...
  async function renderHomepageActiveTab() {
    ...
    } else if (activeHomeTab === 'customers') {
      await renderHomepageCustomers();
    }
  }

  async function renderHomepageCustomers() {
    const tableBody = document.getElementById('homepage-customers-table-body');
    if (!tableBody) return;
    ...
    users.forEach(user => {
      ...
      const dateStr = new Date(user.createdAt).toLocaleString();

      let cType = '';
      let cVal = '';
      if (user.email) {
        cType = '<span style="background:rgba(212,175,55,0.15); color:var(--gold-text); padding:2px 6px; border-radius:4px; font-size:0.75rem; font-weight:600;">Email</span>';
        cVal = user.email;
      } else if (user.phone) {
        cType = '<span style="background:rgba(74,100,88,0.15); color:#4a6458; padding:2px 6px; border-radius:4px; font-size:0.75rem; font-weight:600;">Phone</span>';
        cVal = user.phone;
      }

      tr.innerHTML = `
        <td style="padding: 1.25rem; font-family:monospace; font-size:0.8rem; color:var(--text-secondary);">${user._id}</td>
        <td style="padding: 1.25rem; font-weight:500;">${cVal}</td>
        <td style="padding: 1.25rem;">${cType}</td>
        <td style="padding: 1.25rem; white-space: nowrap; color:var(--text-secondary);">${dateStr}</td>
      `;
  ...
  // --- CUSTOMER ACCOUNTS / AUTHENTICATION LOGIC ---
  const profileBtn = document.getElementById('profile-btn');
  const authModalWrap = document.getElementById('auth-modal');
  const accModalWrap = document.getElementById('account-modal');
  const closeAuthBtn = document.getElementById('close-auth-btn');
  const closeAccBtn = document.getElementById('close-account-btn');
  const authLogoutBtn = document.getElementById('auth-logout-btn');

  const authSendForm = document.getElementById('auth-send-form');
  ...
  const authSentTarget = document.getElementById('auth-sent-target');
  const authBackBtn = document.getElementById('auth-back-btn');
  const userOrdersList = document.getElementById('user-orders-list');

  let currentAuthContact = '';

  if (profileBtn) {
    profileBtn.addEventListener('click', () => {
      const token = localStorage.getItem('ankri_token');
      if (token) {
        // User logged in, show dashboard
        if (accModalWrap) accModalWrap.classList.remove('hidden');
        document.body.style.overflow = 'hidden';
        fetchUserOrders();
      } else {
        // Show login flow
        if (authSendForm) authSendForm.reset();
        if (authVerifyForm) authVerifyForm.reset();
        if (authStep1) authStep1.classList.remove('hidden');
        if (authStep2) authStep2.classList.add('hidden');
        if (authModalWrap) authModalWrap.classList.remove('hidden');
        document.body.style.overflow = 'hidden';
      }
    });
  }
  ...
  if (closeAuthBtn) {
    closeAuthBtn.addEventListener('click', () => {
      authModalWrap.classList.add('hidden');
      document.body.style.overflow = '';
    });
  }
  ...
  if (closeAccBtn) {
    closeAccBtn.addEventListener('click', () => {
      accModalWrap.classList.add('hidden');
      document.body.style.overflow = '';
    });
  }
  ...
  if (authLogoutBtn) {
    authLogoutBtn.addEventListener('click', () => {
      localStorage.removeItem('ankri_token');
      accModalWrap.classList.add('hidden');
  ...
  if (authSendForm) {
    authSendForm.addEventListener('submit', async (e) => {
      e.preventDefault();
      const contactInput = document.getElementById('auth-contact').value.trim();
      const submitBtn = document.getElementById('auth-send-btn');

      const type = contactInput.includes('@') ? 'email' : 'phone';
      currentAuthContact = contactInput;
      ...
      try {
        const res = await fetch('http://localhost:5000/api/auth/send-otp', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ contactMethod: contactInput, type })
        });
        const data = await res.json();

        if (res.ok) {
          authSentTarget.textContent = contactInput;
          authStep1.classList.add('hidden');
          authStep2.classList.remove('hidden');
          if (data.dummyCode) {
            console.warn('[DEV] Bypass active. OTP is:', data.dummyCode);
            showToast(`Development Mode: Internal OTP generated (check console).`);
          }
        } else {
  ...
  if (authVerifyForm) {
    authVerifyForm.addEventListener('submit', async (e) => {
      ...
      const type = currentAuthContact.includes('@') ? 'email' : 'phone';
      ...
      try {
        const res = await fetch('http://localhost:5000/api/auth/verify-otp', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ contactMethod: currentAuthContact, otp: codeInput, type })
        });
        const data = await res.json();

        if (res.ok) {
          localStorage.setItem('ankri_token', data.token);
          authModalWrap.classList.add('hidden');
          accModalWrap.classList.remove('hidden');
          showToast(`Welcome back, ${currentAuthContact}!`);
          fetchUserOrders();
        } else {
  ...
  async function fetchUserOrders() {
    ...
    if (userOrdersList) {
      userOrdersList.innerHTML = `<div style="text-align:center; padding:2rem;"><div style="width:24px; height:24px; border:3px solid rgba(213,195,184,0.3); border-top-color:#d4af37; border-radius:50%; animation:spin 1s linear infinite; margin:auto;"></div></div>`;
    }
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 401) {
          localStorage.removeItem('ankri_token');
          if (accModalWrap) accModalWrap.classList.add('hidden');
          if (authModalWrap) authModalWrap.classList.remove('hidden');
      ...
      if (orders.length === 0) {
        userOrdersList.innerHTML = `<p style="text-align:center; color:var(--text-secondary); padding:3rem 1rem;"><i data-lucide="package-open" style="width:48px; height:48px; opacity:0.3; margin-bottom:1rem; display:inline-block;"></i><br>You haven't placed any orders yet. <br><a href="#collection" style="color:var(--accent); margin-top:1rem; display:inline-block; font-weight:500;" onclick="document.getElementById('close-account-btn').click();">Start Shopping</a></p>`;
        if (window.lucide) window.lucide.createIcons();
        return;
      }
      ...
      orders.forEach(order => {
        ...
        const date = new Date(order.timestamp).toLocaleDateString('en-IN', { year: 'numeric', month: 'short', day: 'numeric' });
        ...
        card.innerHTML = `
          ...
          <div style="display:flex; justify-content:space-between; align-items:center; border-top:1px dashed rgba(213,195,184,0.3); padding-top:1rem;">
            <span style="font-size:0.85rem; padding:0.2rem 0.6rem; border-radius:12px; background: ${order.status === 'Pending' ? 'rgba(212,175,55,0.1)' : 'rgba(46,204,113,0.1)'}; color: ${order.status === 'Pending' ? 'var(--accent)' : '#2ecc71'};">
              ${order.status}
            </span>
            <span class="account-order-total">Total: ₹${order.total}</span>
          </div>
        `;
      ...
      if (userOrdersList) {
        userOrdersList.innerHTML = `<p style="text-align:center; color:red;">Failed to load order history.</p>`;
      }
    }
  }
